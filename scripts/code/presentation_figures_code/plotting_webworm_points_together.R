...
#Plot black and red points together, with no plant hardiness zones


#Get USA and Canada borders
north_america <- ne_countries(continent="north america", country=c("United States of America", "Canada"))
north_america <- spTransform(north_america, crs(points))

tmap_mode("plot")
borders_tmap_black <- tm_shape(north_america, bbox=points)+
  tm_borders(lwd=3)+
...
borders_tmap_red <- tm_shape(north_america, bbox=points)+
  tm_borders(lwd=3)+
...
arrange_borders <- tmap_arrange(borders_tmap_black, borders_tmap_red, ncol=1)
arrange_borders
...
tmap_save(arrange, "figures_for_presentations/colour_morph_distribution_withzones.jpg", units="px", width=3000, height=2300)
